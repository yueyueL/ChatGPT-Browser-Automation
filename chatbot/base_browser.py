...
class BaseBrowser:
    ...
    def __init__(
        ...
        if not skip_login and not self.pass_verification():
            raise RuntimeError('Verification failed, please check your connection.')

        if not skip_login and self.login_type == 'normal':
            self.login(username, password)
        elif not skip_login and self.login_type == 'google':
            self.login_by_google(username, password)
        elif not skip_login and self.login_type == 'manully':
            self.wait_login_manully(100)
    ...
    def wait_login_manully(self, timeout: int):
        '''
